...
import sys 
# sys.path.append('/home/ubuntu/multimodal_mtv')
...
class multiheadattention(nn.Module):
    ...
    def split_heads(self, x, batch_sizes):
        # print(f'before split heads {x.shape}')
        x = torch.reshape(x,[batch_sizes, -1, self.n_head, self.d_k])
        # print(f'after split heads {x.shape}')
...
class Tublet_Embedding(nn.Module):
    ...
    def _init(self):
        init.kaiming_normal_(self.cov_layer.weight,nonlinearity='relu')
        init.constant_(self.cov_layer.bias,0)
    def forward(self, x):
...
class CrossViewAttentionEncoder(nn.Module):
    ...
    def do_mlp_fusion(self, tokens, count):
        #这里采用mlp的融合方法
        view_indices = [2,1]
        view_idx = view_indices[count]
        # print(tokens[0].shape, tokens[1].shape, tokens[2].shape)
        #就是将后一个的输出融合到前一个中
        mlp_back = tokens[view_idx]
        mlp_for = tokens[view_idx - 1]
        # print(mlp_back.shape, mlp_for.shape)
        #将后一个的mlp输出投影到前一个
        mlp_b_f = nn.Linear(in_features=mlp_back.shape[-1], out_features=mlp_for.shape[-1]).to(device)(mlp_back)
        #并进行cat
        # print(mlp_b_f.shape, mlp_for.shape)
        cat_result = torch.cat((mlp_for, mlp_b_f), dim=1)
        #通过mlp输出
        # print(tokens[view_idx-1].shape, cat_result.shape)
        #将cat_result重新进行投影，使其维度与凭借之前的相同
        projected_cat_result = rearrange(nn.Linear(in_features=cat_result.shape[1], out_features=tokens[view_idx - 1].shape[1]).to(device)(rearrange(cat_result, 'b t d -> b d t')), 'b d t -> b t d')
        # print(projected_cat_result.shape)
        y = nn.LayerNorm(normalized_shape=self.embedding_dim_list[view_idx-1]).to(device)(tokens[view_idx - 1] + projected_cat_result)
        tokens[view_idx - 1] = y
        return tokens
    
    def do_botteneck_fusion(self, tokens, count):
        #这里采用botteneck的融合方法
        view_indices = [2,1]
        view_idx = view_indices[count]
        encoder_back_out = tokens[view_idx]
        encoder_for_out = tokens[view_idx - 1]
        #将后一个encoder的输出做编码压缩，这里的压缩采用MLP压缩
        encoder_back_out_squeeze = nn.Linear(in_features=encoder_back_out.shape[-1], out_features=encoder_for_out.shape[-1]).to(device)(encoder_back_out)
        #将降维之后的向量与之前的加起来
        cat_encoder_back_out_squeeze = torch.cat((encoder_for_out, encoder_back_out_squeeze), dim=1)
        #然后正常通过transformer
        y = MultiHeadSelfAttentionBlock(
                embedding_dims=self.embedding_dim_list[view_idx - 1],
                num_heads=self.num_heads_list[view_idx - 1],
                attn_dropout=self.atten_dropot).to(device)(query=cat_encoder_back_out_squeeze, key=cat_encoder_back_out_squeeze, value=cat_encoder_back_out_squeeze)
        tokens[view_idx - 1] = y
    ...
    def forward(self, tokens, cur_layer,count):
        ...
        #不同的模型融合方式
        if self.fusion_method == 'CA':
            tokens = self.do_cross_attention(tokens=tokens,count=count)
        if self.fusion_method == 'MLP':
            tokens = self.do_mlp_fusion(tokens=tokens, count=count)
        if self.fusion_method == 'Botteneck':
            tokens = self.do_botteneck_fusion(tokens=tokens, count=count)
        else:
            raise NotImplementedError('There is no such fusion method, please check your fusion_method!')
...
class MultiviewEncoder(nn.Module):
    def __init__(self, fusion_layer:list,
        ...
        self.tub_embed = nn.ModuleList([Tublet_Embedding(t=16,h=112,w=112,
...
class MultiViewModel(nn.Module):
    def __init__(self,fusion_layer:list,
        ...
        self.ff1 = nn.Linear(in_features=1024, out_features=256)
        self.ff2 = nn.Linear(in_features=256, out_features=256)
    def forward(self, x):
...
if __name__ == '__main__':
    ...
    model = MultiViewModel(fusion_layer=paras_for_multi_view['fusion_layer'],
                           ...
                           view_number=[2,4,8]).to(device)
    view1 = torch.randn(size=(16,3,16,112,112)).to(device)
    view2= torch.randn(size=(16,3,16,112,112)).to(device)
    view3 = torch.randn(size=(16,3,16,112,112)).to(device)
    print(model([view1,view2,view3]).shape)
    # summary(model,(view1,view2,view3))