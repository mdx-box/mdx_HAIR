...
def train(model:torch.nn.Module,
    ...
    for i in range(Epoches):
        ...
        for item, (X_Skeleton, y) in tqdm(enumerate(train_dataloader)):
            ...
            # print(X_Skeleton[:,:,0:3,:].shape)
            X_Skeleton, y = X_Skeleton[:,:,0:3,:].to(torch.float).to(device), y.to(torch.float).to(device)
            optimizer.zero_grad()
            ...
            ddp_loss += loss.item()
            ddp_acc += acc
            if (item+1) % len(train_dataloader) ==0:
                step_loss = ddp_loss /len(train_dataloader)
                step_acc = ddp_acc /len(train_dataloader) 
                with open('E:/mdx/SelfAttention_ActionRecognition/Result/Skeleton_B16_D4_De24_lr00001.txt', 'a+') as f:
